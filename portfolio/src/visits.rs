...
/// One visitor's stay, gathered back out of the log.
///
/// The log is append-only and one line per thing that happened, which is the
/// right shape to write and the wrong shape to read: a single visit is an
/// `arrive`, a `where` that lands whenever the lookup returns, a line per
/// question and a `leave`, interleaved with everybody else's. This puts them
/// back together.
#[derive(Default)]
struct Stay {
    at: u64,
...
impl Stay {
    /// What to call them.
    ///
    /// Over ssh there is a name, because they typed one in front of the `@` --
    /// any string is accepted, so it is what they chose to be called and not an
    /// account. A browser has nowhere to type one, so the stable id it keeps
    /// stands in: not a name, but the thing that makes two visits the same
    /// person. Neither, and they are simply someone.
    fn name(&self) -> String {
        if !self.user.is_empty() {
            return self.user.clone();
        }
        match self.id.chars().count() {
            0 => "someone".to_string(),
            _ => self.id.chars().take(12).collect(),
        }
    }
}

/// `1h02m`, `8m14s`, `9s`. Empty for a visit with no `leave` line, which is one
/// still going or one whose process was killed under it.
fn spell(secs: Option<u64>) -> String {
    let Some(s) = secs else { return "open".to_string() };
...
///
/// Hand-rolled for the same reason `json.rs` is: it is twenty lines against a
/// dependency and a build, and the only date this program formats is this one.
/// The civil-from-days part is Howard Hinnant's, which is the one everybody
/// uses because getting it wrong is a leap year nobody notices for three years.
...
/// The visit log, read back as something a person can read.
///
/// Everything asked of it -- who came, when, for how long, what they asked --
/// has been in the file since the beginning. What was missing was any way to
/// see it without writing `jq` by hand, which meant in practice it was never
/// looked at.
///
/// Deliberately in this file rather than beside `main`. The reader has to agree
/// with the writer about every field name in here, and two files that agree by
/// having been written on the same afternoon stop agreeing on a later one.
pub fn report() -> String {
    let Ok(text) = std::fs::read_to_string(path()) else {
        return format!("no visits yet -- nothing at {}\n", path().display());
    };

    // Insertion-ordered, because a log read back out of order is a worse log.
    let mut order: Vec<String> = Vec::new();
    let mut stays: HashMap<String, Stay> = HashMap::new();
    ...
    for line in text.lines() {
        ...
        let number = |k: &str| v.get(k).and_then(|x| x.as_f64()).unwrap_or(0.0) as u64;
        ...
        match v.get("event").and_then(|e| e.as_str()).unwrap_or_default() {
            "arrive" => {
                stay.at = number("at");
                stay.via = field("via");
                stay.user = field("user");
                stay.id = field("id");
                stay.ip = field("ip");
                stay.client = field("client");
                stay.seen_before = number("seen_before");
            }
            "where" => {
                // A city that is also its own region -- Berlin, Singapore,
                // Delhi -- otherwise says itself twice.
                let mut place: Vec<String> = Vec::new();
                for part in [field("city"), field("region"), field("country")] {
                    if !part.is_empty() && !place.contains(&part) {
                        place.push(part);
                    }
                }
                stay.place = place.join(", ");
            }
            // Both halves are logged, `question` when it is sent and `ask` when
            // it comes back, so listing both would list every answered question
            // twice. The answered ones are collected first and the sent ones
            // are filtered against them below.
            "ask" => {
                let q = field("q");
                answered.entry(session.clone()).or_default().push(q.clone());
                stay.asked.push((q, String::new()));
            }
            "question_status" => {
                let q = field("q");
                answered.entry(session.clone()).or_default().push(q.clone());
                stay.asked.push((q, field("status")));
            }
            "question" => stay.asked.push((field("q"), "unanswered".to_string())),
            "leave" => stay.secs = Some(number("secs")),
            _ => {}
        }
    }

    let mut out = String::new();
    let (mut returning, mut questions) = (0, 0);

    for session in &order {
        let Some(stay) = stays.get(session) else { continue };
        let seen = answered.get(session).cloned().unwrap_or_default();

        // Every question once: the answered ones as themselves, and the sent
        // ones only where nothing ever came back for them.
        let mut shown: Vec<&(String, String)> = Vec::new();
        for entry in &stay.asked {
            let dropped = entry.1 == "unanswered" && seen.contains(&entry.0);
            if !dropped && !shown.iter().any(|s| s.0 == entry.0 && s.1 == entry.1) {
                shown.push(entry);
            }
        }

        if stay.seen_before > 0 {
            returning += 1;
        }
        questions += shown.len();
        ...
        let mut line = format!(
            "{}  {:<4} {}",
            stamp(stay.at),
            if stay.via.is_empty() { "?" } else { &stay.via },
            stay.name()
        );
        for part in [
            stay.ip.clone(),
            ...
            spell(stay.secs),
            match shown.len() {
                0 => "no questions".to_string(),
                1 => "1 question".to_string(),
                n => format!("{n} questions"),
            },
            match stay.seen_before {
                0 => "first time".to_string(),
                n => format!("visit {}", n + 1),
            },
        ] {
            if !part.is_empty() {
        ...
        if !stay.client.is_empty() {
            out.push_str(&format!("      {}\n", stay.client));
        }
        for (q, status) in shown {
            match status.is_empty() {
                true => out.push_str(&format!("      {q}\n")),
                false => out.push_str(&format!("      {q}   [{status}]\n")),
            }
        }
        out.push('\n');
    }

    let span = match (order.first().and_then(|s| stays.get(s)), order.last().and_then(|s| stays.get(s))) {
        (Some(a), Some(b)) if a.at > 0 => format!("  ·  {} to {}", stamp(a.at), stamp(b.at)),
        _ => String::new(),
    };
    out.push_str(&format!(
        "{} visit{}  ·  {returning} returning  ·  {questions} question{}{span}\n",
        order.len(),
...
mod reading {
    ...
    /// The one piece of arithmetic in here that has no second opinion.
    ///
    /// Checked against dates that are wrong in a different way each: the epoch,
    /// both kinds of leap year, and a century that is not one -- 2100 is
    /// divisible by four and is not a leap year, which is the case a
    /// hand-rolled calendar gets wrong and nobody notices for decades.
    #[test]
    ...
    fn how_long_they_stayed_reads_as_a_duration() {
        ...
        // No `leave` line: still here, or the process died under them. Either
        // way it is not a duration and must not be printed as one.
        assert_eq!(spell(None), "open");
    }

    /// A question is listed once, whatever happened to it.
    ///
    /// Both halves are logged -- `question` when it is sent, `ask` when it
    /// comes back -- so the naive read shows every answered question twice and
    /// every cancelled one twice with different endings.
    #[test]
    fn every_question_is_listed_exactly_once() {
        let dir = std::env::temp_dir().join(format!("visits-report-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let log = dir.join("visits.jsonl");
        std::fs::write(&log, concat!(
            r#"{"at":1787600000,"event":"arrive","session":"s1","via":"ssh","user":"prince","id":"k","ip":"203.0.113.7","client":"c","returning":false,"seen_before":0,"first_seen":0}"#, "\n",
            r#"{"at":1787600040,"event":"question","session":"s1","id":"k","q":"answered one"}"#, "\n",
            r#"{"at":1787600055,"event":"ask","session":"s1","id":"k","n":1,"q":"answered one","a":"yes"}"#, "\n",
            r#"{"at":1787600120,"event":"question","session":"s1","id":"k","q":"cancelled one"}"#, "\n",
            r#"{"at":1787600150,"event":"question_status","session":"s1","id":"k","status":"cancelled","q":"cancelled one"}"#, "\n",
            r#"{"at":1787600300,"event":"question","session":"s1","id":"k","q":"never came back"}"#, "\n",
            r#"{"at":1787600494,"event":"leave","session":"s1","secs":494,"turns":1}"#, "\n",
        )).unwrap();

        // `path()` reads this, and the tests in this binary share a process.
        unsafe { std::env::set_var("PORTFOLIO_VISITS", &log) };
        let out = report();
        unsafe { std::env::remove_var("PORTFOLIO_VISITS") };

        assert_eq!(out.matches("answered one").count(), 1, "{out}");
        assert_eq!(out.matches("cancelled one").count(), 1, "{out}");
        assert!(out.contains("cancelled one   [cancelled]"), "{out}");
        assert!(out.contains("never came back   [unanswered]"), "{out}");
        assert!(out.contains("prince"), "the name is missing:\n{out}");
        assert!(out.contains("8m14s"), "how long they stayed is missing:\n{out}");
        assert!(out.contains("3 questions"), "the count is wrong:\n{out}");
